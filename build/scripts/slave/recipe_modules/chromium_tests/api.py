...
class ChromiumTestsApi(recipe_api.RecipeApi):
  ...
  def get_bot_config(self, mainname, buildername):
    ...
    return freeze(main_dict.get('builders', {}).get(buildername))

  def configure_build(self, mainname, buildername, override_bot_type=None):
    main_dict = self.builders.get(mainname, {})
    ...
    # without notice.
    if main_dict.get('settings', {}).get('src_side_runtest_py'):
  ...
  def ensure_checkout(self, mainname, buildername):
    bot_config = self.get_bot_config(mainname, buildername)
  ...
  def set_up_swarming(self, mainname, buildername):
    bot_config = self.get_bot_config(mainname, buildername)
  ...
  def runhooks(self, mainname, buildername, update_step):
    bot_config = self.get_bot_config(mainname, buildername)
  ...
  def get_main_dict_with_dynamic_tests(
      self, mainname, buildername, test_spec, scripts_compile_targets):
    # We manually thaw the path to the elements we are modifying, since the
    # builders are frozen.
    main_dict = dict(self.builders[mainname])
    ...
    for loop_buildername in builders:
      ...
      builders[loop_buildername]['tests'] = (
          self.generate_tests_from_test_spec(
              self.m, test_spec, builder_dict, loop_buildername, mainname,
    ...
    return freeze(main_dict)

  def prepare_checkout(self, mainname, buildername):
    bot_config = self.get_bot_config(mainname, buildername)

    update_step = self.ensure_checkout(mainname, buildername)
    # TODO(robertocn): Remove this hack by the end of Q1/2016.
    if (mainname == 'tryserver.chromium.perf'
    ...
    self.set_up_swarming(mainname, buildername)
    self.runhooks(mainname, buildername, update_step)

    test_spec = self.get_test_spec(mainname, buildername)
    ...
    main_dict = self.get_main_dict_with_dynamic_tests(
        mainname, buildername, test_spec, scripts_compile_targets)
  ...
  def generate_tests_from_test_spec(self, api, test_spec, builder_dict,
      buildername, mainname, enable_swarming, scripts_compile_targets,
      generators):
    tests = builder_dict.get('tests', ())
    # TODO(phajdan.jr): Switch everything to scripts generators and simplify.
    for generator in generators:
      tests = (
          tuple(generator(api, mainname, buildername, test_spec,
  ...
  def get_compile_targets_and_tests(
      self, mainname, buildername, main_dict, test_spec,
    ...
    for _, builder_dict in main_dict.get(
        'builders', {}).iteritems():
  ...
  def compile_specific_targets(
      self, mainname, buildername, update_step, main_dict,
    ...
    if bot_type == 'builder':
      if (mainname == 'chromium.linux' and
      ...
      for loop_buildername, builder_dict in sorted(main_dict.get(
          'builders', {}).iteritems()):
        if builder_dict.get('parent_buildername') == buildername:
          self.m.trigger({
  ...
  def run_mb_and_compile(self, compile_targets, isolated_targets, name_suffix):
    if self.m.chromium.c.project_generator.tool == 'mb':
      # We don't use the mainname and buildername passed in, because
      # those may be the values of the continuous builder the trybot may
      # be configured to match; we need to use the actual mainname
      # and buildername we're running on, because it may be configured
      # with different MB settings.
      real_mainname = self.m.properties['mainname']
      real_buildername = self.m.properties['buildername']
      self.m.chromium.run_mb(real_mainname, real_buildername,
  ...
  def tests_for_builder(self, mainname, buildername, update_step, main_dict,
    ...
    if bot_type == 'tester':
      ...
      if specified_url:
        ...
        legacy_build_url = self._make_legacy_build_url(main_config,
                                                       mainname)
  ...
  def _make_legacy_build_url(self, main_config, mainname):
    return self.m.archive.legacy_download_url(
               ...
               extra_url_components=(
                   ...
                   else self.m.properties['mainname']))

  @contextlib.contextmanager
  def wrap_chromium_tests(self, mainname, tests=None):
    ...
    if require_device_steps:
      ...
      self.m.chromium_android.common_tests_setup_steps(
          perf_setup=(mainname.startswith('chromium.perf') or
                      mainname.startswith('tryserver.chromium.perf')))
    ...
    try:
      ...
      if require_device_steps:
        ...
        if mainname in ('chromium.linux', 'tryserver.chromium.linux'):
  ...
  def run_tests_and_deapply_as_needed(self, mainname, api, tests,
                                      bot_update_step):
    ...
    with self.wrap_chromium_tests(mainname, tests):
  ...
  def get_common_args_for_scripts(self):
    ...
    for name in ('buildername', 'subordinatename', 'buildnumber', 'mainname'):
  ...
  def should_force_legacy_compiling(self, main, builder):
    ...
    Args:
      main (str): The main name used as a key on mb_config.pyl
