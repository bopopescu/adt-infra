...
def RunSteps(api):
  ...
  # rmwildcard demo
  api.file.rmwildcard('*.o', api.path['subordinate_build'])
  ...
  try:
    ...
    tmp_dir = api.path['subordinate_build'].join('copytree_example_tmp')
    api.file.makedirs('makedirs', tmp_dir)
    path = tmp_dir.join('dummy_file')
    api.file.write('write %s' % path, path, content)
    new_tmp = api.path['subordinate_build'].join('copytree_example_tmp2')
