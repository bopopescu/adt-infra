...
class PerfTryJobApi(recipe_api.RecipeApi):
  ...
  def _run_perf_job(self, perf_cfg, bot_update_step, main_dict):
    ...
    results_with_patch = self._build_and_run_tests(
        test_cfg, bot_update_step, main_dict, r[0],
    ...
    results_without_patch = self._build_and_run_tests(
        test_cfg, bot_update_step, main_dict, r[1],
  ...
  def run_cq_job(self, update_step, main_dict, files_in_patch):
    ...
    self._compile('With Patch', self.m.properties['mainname'],
                  self.m.properties['buildername'], update_step, main_dict)
  ...
  def _checkout_revision(self, update_step, main_dict, revision=None):
  ...
  def _compile(self, name, mainname, buildername, update_step,
               main_dict, test_spec=None):
  ...
  def _build_and_run_tests(self, cfg, update_step, main_dict, revision,
                           **kwargs):
    """Compiles binaries and runs tests for a given a revision."""
    update_step = self._checkout_revision(update_step, main_dict, revision)
