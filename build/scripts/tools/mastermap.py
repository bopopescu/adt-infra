...
PORT_TYPE_MAP = {
  'port': Main.Base.MASTER_PORT,
  'subordinate_port': Main.Base.SLAVE_PORT,
  'alt_port': Main.Base.MASTER_PORT_ALT,
}
...
def main_audit(mains, output, opts):
  ...
  for main in mains:
    for port_type, port_digit in PORT_TYPE_MAP.iteritems():
      if not str(main[port_type]).startswith(str(port_digit)):
        ret = 1
  ...
  # Look for mains whose port, subordinate_port, alt_port aren't separated by 10000.
  bad_sep_mains = []
  for main in mains:
    if (getint(main['subordinate_port']) - getint(main['port']) != 10000 or
        getint(main['alt_port']) - getint(main['subordinate_port']) != 10000):
      ret = 1
...
def build_port_str(main_class, port_type, digits):
  ...
  port = '%d%02d%02d' % (port_base, main_class.main_port_base, digits)
